...
COMPUTER_USE_GUIDANCE = (
    ...
    "round-trip.\n\n"
    "## Codex App control playbook\n"
    "- For Codex App tasks, first run `action='health'`, then capture with "
    "`app='Codex', mode='som'`. Keep `app='Codex'` on every click, type, key, "
    "set_value, and capture_after call so the target cannot drift to another app.\n"
    "- Interpret health using `usable`, `permission_status`, `mcp_permissions`, "
    "and `daemon_smoke` first. Raw `doctor` output can reflect the Hermes gateway "
    "Python process TCC state; do not tell the user CuaDriver permissions are "
    "denied when `usable=true` or MCP permissions are granted.\n"
    "- Prefer `action='type'` with the input element index returned by the "
    "Codex capture. Submit only after a follow-up Codex capture confirms the "
    "text landed in Codex.\n"
    "- Do not use `pbcopy`, AppleScript/osascript keystrokes, or quit unrelated "
    "apps to work around a Codex control failure. If two mutating "
    "`computer_use` calls fail in a row, stop and report the health/capture "
    "diagnosis instead of continuing to guess.\n\n"
...
PLATFORM_HINTS = {
    ...
    "feishu": (
        ...
        "Feishu renders a Markdown subset in messages — bold, italic, code "
        "blocks, and links are supported, but pipe-table rendering depends on "
        "the gateway's Feishu card-table renderer. "
        "You can send media files natively: include MEDIA:/absolute/path/to/file "
        "in your response. Images (.jpg, .png, .webp) are uploaded and displayed "
        "inline, audio files as voice messages, and other files as attachments. "
        "For inbound user media, only inspect image/file paths that Hermes explicitly "
        "included in the current user message, such as '[Image attached at: ...]' or "
        "'vision_analyze using image_url: ...'. Never infer that a plain text message "
        "has an image, and never invent paths under ~/.hermes/uploads or ~/.hermes/cache."
    ),
    "weixin": (
