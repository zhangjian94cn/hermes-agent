...
# Feishu post-type 'md' elements do not render tables. Also kept as a
# backwards-compatible alias because external callers reference it.
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _build_outbound_payload(
        ...
        # Local fork (protected feature): opt-in card_table rendering turns
        # markdown tables into Feishu interactive cards
        # (HERMES_FEISHU_MARKDOWN_TABLE_RENDERING / adapter extra
        # "markdown_table_rendering"). Checked before prefer_post so chunked
        # table replies still render as cards.
        if (
            self._markdown_table_rendering == _MARKDOWN_TABLE_RENDER_CARD
            and _MARKDOWN_TABLE_RE.search(content)
        ):
